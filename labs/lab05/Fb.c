...
bool FbUnfriend(Fb fb, char *name1, char *name2) {
    ...
    //find name1 index and name 2 index
    int name1_index = nameToId(fb, name1);
    int name2_index = nameToId(fb, name2);
    
    //same person, cannot be unfriend
    if (name1_index == name2_index) {
        return false;
    }
    
    else if (fb->friends[name1_index][name2_index]) {
        //unfriend
        fb->friends[name1_index][name2_index] = false;
        fb->friends[name2_index][name1_index] = false;
        //successfully unfriend
        return true;
    }
    else {
        return false;
...
List FbMutualFriends(Fb fb, char *name1, char *name2) {
    ...
    for (int i = 0; i < numPeople; i++) {
        ...
        //if is friend with both, append to the return list
...
void FbFriendRecs1(Fb fb, char *name) {
    ...
    int target = nameToId(fb, name);
    //largest number of mutual friends two people could have is numnpeople-2
    int largestMutual = fb->numPeople -2;
    
    printf("%s's friend recommendations\n", name);
    
    // print in order of highest mutuals to lowest mutuals
    for (int i = largestMutual; i > 0; i--) {

        for (int j = 0; j < fb->numPeople; j++) {

            // exclude target and direct friends of the target
            if (j == target || FbIsFriend(fb, name, fb->names[j])) {
                continue;
            }
            ...
            //condition to make sure every recommendations only print once
