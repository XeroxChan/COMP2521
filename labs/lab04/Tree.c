...
List TreeSearchBetween(Tree t, Record lower, Record upper) {
    ...
    doTreeSearchBetween(t, t->root, lower, upper, resultList);
...
static void doTreeSearchBetween(Tree t, Node n, Record lower,
    ...
    //variables to check if the value is out of range
    int lowerCmp = t->compare(lower, n->rec);
    int upperCmp = t->compare(upper, n->rec);
    
    //when value is straightly smaller than the lower bound, go right recursively (no need to go left anymore)
    if (lowerCmp > 0) {
        doTreeSearchBetween(t, n->right, lower, upper, l);
    }
    
    //when value is straightly larger than the upper bound, go left recursively (no need to go right anymore)
    else if (upperCmp < 0) {
        doTreeSearchBetween(t, n->left, lower, upper, l);
    }
    
    //when the value is inside the bound
    else {
        doTreeSearchBetween(t, n->left, lower, upper, l);
        //append to list
        ListAppend(l, n->rec);
